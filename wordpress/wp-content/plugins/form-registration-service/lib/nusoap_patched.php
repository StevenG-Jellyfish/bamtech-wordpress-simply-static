...
/*
 * Library significantly patched to interact with Shire services.
 * Patches include:
 *
 * - Changed Envelope element names in all XML from SOAP-ENV to soapenv.
 * - Added code to remove proxy headers.
 * - Changed $tr->setEncoding('gzip, deflate') to $tr->setEncoding('utf-8'); and
 *   added $tr->setContentType('text/xml') to requests.
 * - Access $this->schemas[$ns] as an array of objects (using [0]) rather than
 *   an object within fetchWSDL()
 * - Replaced Header & Namespaces with Shire services expectations
 *
 */
...
class nusoap_base {
    ...
    */
	...
	'<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tem="http://tempuri.org/" xmlns:int="http://schemas.datacontract.org/2004/07/InternationalWebRegistrationSvc">'.
	'<soapenv:Header/>'.
	"<soapenv:Body>".
		$body.
	"</soapenv:Body>".
...
class nusoap_fault extends nusoap_base {
	...
	*/
	function serialize(){
		...
		/**** Patched: soapenv, header & namespaces ****/
		$return_msg =
			'<?xml version="1.0" encoding="'.$this->soap_defencoding.'"?>'.
...
class wsdl extends nusoap_base {
		            ...
		            foreach ($xs->imports as $ns2 => $list2) {
		                for ($ii = 0; $ii < count($list2); $ii++) {
		                	if (! $list2[$ii]['loaded']) {
		                		...
		                		/**** Patched: Used [0] in $this->schemas[$ns] as this is an array ****/
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
			...
			/**** Patched: Amended encoding type + added Content Type ****/
			$tr->setEncoding('utf-8');
			$tr->setContentType('text/xml');
			/**** /Patched ****/
